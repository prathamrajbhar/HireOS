    ...
    <div className="space-y-6 animate-in fade-in duration-300 pb-16">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-white/45 pb-5">
        <div>
          ...
          <h1 className="text-2xl font-black text-slate-900 tracking-tight">Question Bank & Templates</h1>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="rounded-3xl border border-white/60 bg-white/45 p-6 sm:p-8 shadow-md backdrop-blur-md glass-panel space-y-6">
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-xs font-black uppercase tracking-wider transition-all cursor-pointer ${
                    selectedRole === role
                      ...
                      : 'bg-white/70 text-slate-500 hover:text-slate-800 border border-slate-200/80'
                  }`}
                >
                  {role}
                </button>
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center bg-slate-50/50 p-4 rounded-2xl border border-slate-105">
                <span className="text-[10px] font-bold text-slate-500 uppercase">Assessment Focus Weights</span>
                <div className="flex gap-4 text-xs font-black text-slate-800 uppercase">
              ...
              <div className="space-y-3">
                <h4 className="text-xs font-bold text-slate-800">Calibrated Evaluation Prompts</h4>
                <div className="space-y-2">
                  {currentTemplate.questions.map((item, idx) => (
                    <div key={idx} className="p-4 rounded-2xl border border-slate-100 bg-white/35 flex justify-between items-start gap-4">
                      <div className="space-y-1">
                        <p className="text-xs font-bold text-slate-800 leading-relaxed">{item.q}</p>
                        <div className="flex gap-2 text-[9px] font-bold text-slate-400">
                          <span className="uppercase text-purple-700 bg-purple-50 px-1.5 py-0.2 rounded border border-purple-100">{item.cat}</span>
                          <span>Difficulty: {item.diff}</span>
                        </div>
                      </div>
                      <button className="text-[10px] font-bold text-purple-650 hover:underline cursor-pointer">
                        Edit
                      </button>
                    </div>
                  ))}
        ...
        <div className="space-y-6">
          <div className="rounded-3xl border border-white/60 bg-white/45 p-6 shadow-md backdrop-blur-md glass-panel space-y-4">
            <h3 className="text-xs font-bold text-slate-800 border-b border-slate-100 pb-2.5 flex items-center gap-1.5">
              <Brain className="h-4.5 w-4.5 text-purple-550 animate-pulse" />
              AI Question Generator
            </h3>

            <form onSubmit={handleGenerate} className="space-y-3">
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-455 uppercase tracking-wider block">Topic / Domain</label>
                <input
                  type="text"
                  value={topic}
                  onChange={(e) => setTopic(e.target.value)}
                  placeholder="e.g. System Design, React"
                  className="w-full px-3 py-2 text-xs rounded-xl border border-slate-205 bg-white/70 focus:outline-none focus:border-purple-500 font-semibold"
                />
              </div>

              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-455 uppercase tracking-wider block">Difficulty</label>
                <select
                  value={difficulty}
                  onChange={(e) => setDifficulty(e.target.value)}
                  className="w-full p-2 text-xs rounded-xl border border-slate-205 bg-white/70 focus:outline-none focus:border-purple-505 font-semibold cursor-pointer"
                >
            ...
            {successMsg && (
              <div className="p-3 bg-emerald-50 border border-emerald-100 rounded-2xl text-[10px] font-bold text-emerald-800 text-center">
                {successMsg}
              </div>
            )}
            ...
            {generatedQ && (
              ...
              <div className="p-4 rounded-2xl border border-purple-100 bg-purple-50/20 space-y-3 animate-in fade-in duration-200">
                <h4 className="text-[11px] font-black text-purple-900 uppercase tracking-wide">Generated Question Draft</h4>
                <p className="text-xs text-slate-800 leading-relaxed font-bold">{generatedQ.question}</p>
                
                <div className="text-[10px] text-slate-500 font-semibold space-y-1.5 border-t border-purple-100/50 pt-2.5">
                  <p><strong className="text-slate-700 font-bold block">Evaluation rubric:</strong> {generatedQ.rubric}</p>
                  <p className="italic"><strong className="text-slate-750 font-bold block mt-1">Interviewer hint:</strong> {generatedQ.tip}</p>
                </div>

                <button
                  onClick={handleAddToBank}
                  className="w-full rounded-xl bg-purple-600 hover:bg-purple-750 text-white font-bold py-2 text-xs shadow-sm cursor-pointer mt-1 text-center"
                >
